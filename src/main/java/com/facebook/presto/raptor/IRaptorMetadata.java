...
public class IRaptorMetadata extends RaptorMetadata
{
    ...
    {
        IRaptorTableLayoutHandle raptorHandle = checkType(handle, IRaptorTableLayoutHandle.class, "handle");
    ...
    private ConnectorTableLayout getTableLayout(ConnectorSession session, RaptorTableHandle handle, TupleDomain<ColumnHandle> constraint)
    {
        if (!handle.getDistributionId().isPresent()) {
            return new ConnectorTableLayout(new IRaptorTableLayoutHandle(handle, constraint, Optional.empty()));
        }
        ...
        IRaptorPartitioningHandle partitioning = getPartitioningHandle(handle.getDistributionId().getAsLong(), bucketColumnTypes);

        boolean oneSplitPerBucket = handle.getBucketCount().getAsInt() >= getOneSplitPerBucketThreshold(session);

        return new ConnectorTableLayout(
                new IRaptorTableLayoutHandle(handle, constraint, Optional.of(partitioning)),
                Optional.empty(),
                TupleDomain.all(),
    ...
    private IRaptorPartitioningHandle getPartitioningHandle(long distributionId, List<Type> bucketTypes)
    {
        return new IRaptorPartitioningHandle(distributionId, shardManager.getBucketAssignments(distributionId), bucketTypes);
    }
    ...
    {
        Optional<IRaptorPartitioningHandle> partitioning = layout
                .map(ConnectorNewTableLayout::getPartitioning)
                .map(handle -> checkType(handle, IRaptorPartitioningHandle.class, "partitioning"));
